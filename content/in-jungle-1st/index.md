...
![사진](./enough-today.jpeg)*10줄을 입력한 후의 나, '이정도면 오늘 할 일 다했다!'*
...
### <center>**"정글러가 소개하는 새로운 소통방식, 모정숲!"** (모여봐요, 정글의 숲)</center>

<br>

![사진](./mojung-login.png)*로그인페이지, 대나무의 색과 '크래프톤 정글'의 아이덴티티를 살리고자 노력했다*
...
![사진](./mojung-main.png)*모정숲 게시판 페이지 (연두색 그림자가 포인트)*
...
핵심 기능은 보이는 그대로가 전부다. 메시지/투표 를 만들어 게시글을 생성할 수 있고, 최신 순으로 정렬되어 조회할 수 있으며,
이 게시글에 대해 '반응' 이모지를 달 수 있다. 이모지 종류를 뭘로 할까 고민했는데, 슬랙/디스코드 채널에서 많이 쓰일 법한 이모지를 팀원끼리 추려서 6가지로 확정했다. 시간이 더 있었더라면, '인기순' 정렬을 만들었어도 무척이나 좋았을 것 같다.

<br><br>

![사진](./mojung-vote.PNG)*투표를 완료한 상태의 경우 이렇게 바뀐다*

<br>

이 깔끔한 디자인을 만들기 위해, tooltip 구현을 통한 '내가 선택한 답변' 구현이나 옵션과 Progress Bar 한 줄 정렬 등을 고려했고 구현하려 했지만, 어려움이 있어 현재의 상태가 되었다. 보다시피 CSS Custom을 통해 구현된 것을 알 수 있다.(ㅎㅎ) 물론 Progress Bar와 Card는 Bootstrap을 사용했다.

Current Page를 프론트에 저장해두어서, 이전 다음 페이지 이동을 구현하였고 로그인을 하면 읽던 페이지에서 이어 읽을 수 있는 것도 가능하다.


<br>

![사진](./mojung-figma.png)*피그마로 작업한 디자인*

<br>

사실 피그마를 많이 다뤄보거나 하지는 않았는데, 막상 해야하는 상황에 닥치고보니 쓱쓱 하게 된다는 걸 알았다. 디자인과 유사하게 만드려고 정말 노력했다. _(사람이 절박하면 하게 되더라 ...)_

<br><br>

### **최종 발표 후기**
  위의 설명한 목표 기능까지 완성을 한 후, 최종 발표까지 마쳤다. 

  코치님들의 매서운 지적(혹평)은 어느정도 예상하고 있었지만, 끝나고나니 내가 정말 애송이 같은 생각을 하고 있었구나, 하는 생각을 했다. 잠도 못자가면서 열심히 만들었는데, 써보면 이 짧은 시간에 얼마나 내부 구현과 처리를 잘 했고, 인터랙션에 신경쓰고, 직관적으로 보이게 디자인과 사소한 디테일을 맞추었는지 알텐데... 라면서 말이다.
...
여담이지만, 토큰 구현을 통해 1시간이 지나면 로그인 유지 시간 만료, 30일이 지나면 강제 로그아웃하는 기능(..)도 있었다. 아무도 몰랐겠지만,... (여기에서라도 자랑하고 싶었다)

기능자체는 대단한 건 아니지만 짧은 시간에, 로그인/비로그인 상황에 대한 유저가 당연하다고 느낄 법한 시나리오(자동 로그인/페이지 이동 제한)를 구현해 두었다는게 내 마음속에만 남아있는 자부심이다. 프론트 페이지 구성에 익숙하지 않은 팀원들의 페이지의 디자인과 버그 모두 다 (상의 하에) 내가 수정하고 갈아엎었다. 수고했다 내자신...⭐
...
- **어려웠던 챌린지 사례를 메인 게시글 조회를 위해 따로 만든 테이블의 이야기를 들었는데, 기술적으로 아쉽다, 다른 사례들을 찾아보고 적용했으면 어땠을까**

    ![사진](./mojung-challenge.png)

    메인 게시글의 종류가 메시지/투표 로 다른 상태에서, 각각의 구성 요소와 내용이 다른 것을 어떻게 한 번에 가져올지, 에 대한 이슈가 있다. 팀원들과 회의한 결과, 이들의 type을 구분하되 같이 보관하는 테이블을 만들자는 것이 최선이다. 하고 나름의 결론을 내렸었다. 하지만 이것이 역시나 기술적으로 미흡했던 것 같다. 사실 이조차도 transaction 처리가 되지 않아 언제든지 문제가 생길 수 있는 기능이기는 했다. 이 점은 나도 무척이나 아쉬울 따름이다.

    마지막으로 하나 더 느낀 것이 있다면, **어설프게 하면 그게 무조건 티가 나는 것 같다.** 그리고 이를 무조건 눈치챌 수밖에 없다. 어줍잖게 완성했다면, 이를 알리는 것이 오히려 우리 팀의 약점이 되겠구나, 라는 생각까지 하였다. 드러낼 것은 과감하게 드러내되 자신있게 보일 수 있을 정도여야 하고, 아니라면 철저히 감추는게 완성도나 여러 측면에서 좋은 것 같다. 
...
![사진](./last-me.png)*왜 안되는거야, 미치겟네 왜 안돼??! 그치면 코딩은 좋아...*

<br>

하지만, 코딩하는게 늘 즐겁지는 않다. 좋다가도 싫고 싫다가도 좋은게 이것 같다. (괴로워) 

그래도 언젠가는, 정말로 '하고 싶어서 하는 사람'이 되고 싶다. 적어도 현재는 그런 기분은 모르기 때문이다.
